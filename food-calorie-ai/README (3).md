...
graph TB
    ...
    WEB -->|영양 정보 조회| SQLITE
    SQLITE -->|8가지 영양소| WEB
    WEB <-->|식단 기록 저장/조회| FIREBASE
    WEB -->|영양 정보 + 식단 통계| USER
```
...
sequenceDiagram
    ...
    alt 사진 인식
        User->>Web: 음식 사진 업로드
        Web->>Model: 이미지 전처리 후 추론
        Model->>Web: 음식명 + 신뢰도
    else 바코드 인식
        User->>Web: 바코드 촬영 (pyzbar)
        Web->>API: 바코드 번호 전송
        API->>Web: 식품명 반환
    end
...
```mermaid
flowchart TD
    A[공공데이터 CSV 다운로드] --> B[nutrition_db.py 실행]
    B --> C[pandas로 CSV 로드\ncp949 인코딩]
    C --> D[필요 컬럼 추출 및\n숫자형 변환 · NaN 처리]
    D --> G[(SQLite\nfood_nutrition.db\n약 5만 건)]
